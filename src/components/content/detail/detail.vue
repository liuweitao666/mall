<template>
  <div class="detail">
    ...
    <BScrolls class="detail-scroll" ref="Scroll" @onscroll="onscroll">
      ...
      <Recommend :recommends="recommend" ref="recommend" @imgload="imgloaded"></Recommend>
    </BScrolls>
...
export default {
  ...
  created() {
    ...
    this.getoffsettop = debounce(() => {
      ...
      this.themeTopYs.push(this.$refs.detail.$el.offsetTop - 44);
  ...
  methods: {
    ...
    onscroll(position) {
      ...
      for (let i = 0; i < length; i++) {
        if (
          ...
          -position.y >= this.themeTopYs[length - 1]
        ) {
          this.CurrentIndex = i;
        }
      }
    ...
    joincart() {
      ...
      this.$store.dispatch("pushCart", pushCart).then(res => {
        // console.log(res)
